"""
Wav2Small2.0 architecture (arousal / dominance / valence), ported VERBATIM from
the official model card so the published weights load unchanged.

Source: https://huggingface.co/audeering/wav2small  (paper: arXiv:2408.13920)
LICENSE: CC BY-NC-SA 4.0 -- RESEARCH ONLY, not commercially usable. Used here as
a benchmark candidate to characterize signal quality, NOT shipped in the SDK.

~17K params. Input: raw 16 kHz mono waveform [batch, samples].
Output: logits [batch, 3] = (arousal, dominance, valence) in [0, 1].
"""
import librosa
import numpy as np
import torch
...
HF_ID = "audeering/wav2small"


def load(device: str = "cpu") -> Wav2Small:
    return Wav2Small.from_pretrained(HF_ID).to(device).eval()


def predict_adv(model: Wav2Small, samples_16k, device: str = "cpu") -> np.ndarray:
    """Raw 16 kHz mono float samples -> np.array([arousal, dominance, valence])."""
    sig = torch.from_numpy(np.asarray(samples_16k, dtype=np.float32))[None, :].to(device)
    with torch.no_grad():
        logits = model(sig)
